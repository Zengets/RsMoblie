...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/sparechangehistory'],
}))
class SpareChangeHistory extends React.Component {
        ...
        this.setState({
            ...
            this.setNewState(postUrl, postData, () => {
                this.setNewState("done", "0", () => {
                    ...
                    if (refreshing) {
                        this.setState({
                            resData: [{ items: this.props.index.sparechangehistory.list }],
                            refreshing: false,
                            isLoadMore: false
                        })
                    } else {
                        this.setState({
                            resData: this.state.resData.concat([{ items: this.props.index.sparechangehistory.list }]),
                            isLoadMore: false
                        })
                    }
                })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.sparechangehistory.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.sparechangehistory.pageNum + 1
                }
            }, () => {
                this.getData()
            })
    ...
    render() {
        ...
        let renderItem = ({ section: section, row: row }) => {
            ...
            return item ? <SpareChangeMissionItem item={item} navigation={this.props.navigation} type="history"></SpareChangeMissionItem> : <View></View>
        }
        ...
        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.sparechangehistory.hasNextPage}
...
export default SpareChangeHistory