...
import { SafeAreaViewPlus, OneToast, Header, Modal, TitleSearch, ChatItem } from '../../../components';
...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/ChatqueryList'],
}))
class ChatList extends React.Component {
        ...
        this.setState({
            ...
            this.setNewState(postUrl, postData, () => {
                this.setNewState("done", "0", () => {
                    ...
                    if (refreshing) {
                        this.setState({
                            resData: [{ items: this.props.index.ChatqueryList.list }],
                            refreshing: false,
                            isLoadMore: false
                        })
                    } else {
                        this.setState({
                            resData: this.state.resData.concat([{ items: this.props.index.ChatqueryList.list }]),
                            isLoadMore: false
                        })
                    }
                })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.ChatqueryList.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.ChatqueryList.pageNum + 1
                }
            }, () => {
                this.getData()
            })
    ...
    render() {
        ...
        let renderItem = ({ section: section, row: row }) => {
            ...
            return item ? <ChatItem item={item} navigation={this.props.navigation} type="history"></ChatItem> : <View></View>
        }
        ...
        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.ChatqueryList.hasNextPage}
