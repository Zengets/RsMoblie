...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/upkeepmission'],
}))
class UpkeepMission extends React.Component {
    constructor(props) {
        super(props);
        let { key, value,equipmentNo } = props.route.params ? props.route.params : { key: "", value: "",equipmentNo:"" }
        this.state = {
            isLoadMore: true,
            height: new Animated.Value(45),
            refreshing: true,
            postUrl: "upkeepmission",
            search: true,
            showbtn: false,
            postData: key ? {
                "pageIndex": "1",  //--------页码*
        ...
        this.setState({
            ...
            this.setNewState(postUrl, postData, () => {
                this.setNewState("done", "0", () => {
                    ...
                    if (refreshing) {
                        this.setState({
                            resData: [{ items: this.props.index.upkeepmission.list }],
                            refreshing: false,
                            isLoadMore: false
                        })
                    } else {
                        this.setState({
                            resData: this.state.resData.concat([{ items: this.props.index.upkeepmission.list }]),
                            isLoadMore: false
                        })
                    }
                })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.upkeepmission.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.upkeepmission.pageNum + 1
                }
            }, () => {
                this.getData()
            })
    ...
    render() {
        ...
        let renderItem = ({ section: section, row: row }) => {
            ...
            return item ? <UpkeepItem item={item} navigation={this.props.navigation} type="mission"></UpkeepItem> : <View></View>
        }

        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            <Header
                navigation={navigation}
                title={title?title:"维保任务"}
                rightwidth={70}
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.upkeepmission.hasNextPage}
