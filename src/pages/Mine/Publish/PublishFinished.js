...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/publishfinish'],
}))
class PublishFinished extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoadMore: true,
        ...
        this.setState({
            ...
            this.setNewState(postUrl, postData, () => {
                this.setNewState("done", "0", () => {
                    ...
                    if (refreshing) {
                        this.setState({
                            resData: [{ items: this.props.index.publishfinish.list }],
                            refreshing: false,
                            isLoadMore: false
                        })
                    } else {
                        this.setState({
                            resData: this.state.resData.concat([{ items: this.props.index.publishfinish.list }]),
                            isLoadMore: false
                        })
                    }
                })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.publishfinish.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.publishfinish.pageNum + 1
                }
            }, () => {
                this.getData()
            })
        ...
        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.publishfinish.hasNextPage}
...
export default PublishFinished