...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/noticetoconfirm'],
}))
class NoticeToConfirm extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoadMore: true,
        ...
        this.setState({
            ...
            this.setNewState(postUrl, postData, () => {
                this.setNewState("done", "0", () => {
                    ...
                    if (refreshing) {
                        this.setState({
                            resData: [{ items: this.props.index.noticetoconfirm.list }],
                            refreshing: false,
                            isLoadMore: false
                        })
                    } else {
                        this.setState({
                            resData: this.state.resData.concat([{ items: this.props.index.noticetoconfirm.list }]),
                            isLoadMore: false
                        })
                    }
                })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.noticetoconfirm.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.noticetoconfirm.pageNum + 1
                }
            }, () => {
                this.getData()
            })
        ...
        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.noticetoconfirm.hasNextPage}
...
export default NoticeToConfirm