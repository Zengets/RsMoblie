...
import { SafeAreaViewPlus, OneToast, Header, Modal, TitleSearch, KnowledgeItem } from '../../components';
...
@connect(({ index, loading }) => ({
    index,
    submitting: loading.effects['index/queryKnowledgeByFaultId'],
}))
class KnowledgeinDevice extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoadMore: true,
            ...
            refreshing: true,
            postUrl: "queryKnowledgeByFaultId",
            search: true,
            showbtn: false,
            postData: {
                "pageIndex": "1",  //--------页码*
                "pageSize": "10",  //--------每页条数*
                "faultId": props.route?.params?.id,//设备类型id，筛选条件
                "knowledgeBaseName": "",//文件名，筛选条件
                "purposeType": "",//用途类型，筛选条件
                "equipmentTypeId":""
            },
            resData: [{ items: [] }]
        }
    ...
    //获取数据
    getData() {
        //this.setNewState("queryKnowledgeByFaultId", { faultId: this.props.index.repairstep.faultType })
        this.setState({
            postData: {
                ...this.state.postData,
        ...
        if (done == "1" && formdata.length > 0) {
            this.setState({
                postData: {
                    "pageIndex": "1",  //--------页码*
                    "pageSize": "10",  //--------每页条数*
                    "faultId": this.props.route?.params?.id,//设备类型id，筛选条件
                    "equipmentTypeId": getVal("equipmentTypeId"),//设备类型id，筛选条件
                    "knowledgeBaseName": getVal("knowledgeBaseName"),//文件名，筛选条件
                    "purposeType": getVal("purposeType"),//用途类型，筛选条件
                },
            }, () => {
                this.onRefresh()
            })
    ...
    //上拉加载
    pullUpLoading = () => {
        if (!this.state.isLoadMore && this.props.index.queryKnowledgeByFaultId.hasNextPage) {
            this.setState({
                isLoadMore: true,
                postData: {
                    ...this.state.postData,
                    pageIndex: this.props.index.queryKnowledgeByFaultId.pageNum + 1
                }
            }, () => {
                this.getData()
            })
    ...
    render() {
        ...
        let searchprops = {
            ...
            navigation,
            placeholder: "输入文件名查询...",
            value: postData.knowledgeBaseName,
            onChangeText: (val, ifs) => {
                this.setState({
                    ...
                    this.changeData("knowledgeBaseName", val)
                    if (ifs) {
                        this.onRefresh()
                    }
                })
            ...
            handleFormData: (fn) => {
                let formdatas = [{
                    ...
                    value: "",
                    placeholder: "请选择用途类型",
                    option: res.purposeTypeList && res.purposeTypeList
                }]
        ...
        let renderItem = ({ section: section, row: row }) => {
            ...
            return item ? <KnowledgeItem item={item} navigation={this.props.navigation}></KnowledgeItem> : <View></View>
        }
        ...
        return <SafeAreaViewPlus loading={submitting && isLoadMore && refreshing}>
            ...
            <View flex >
                ...
                <LargeList
                    ...
                    allLoaded={!this.props.index.queryKnowledgeByFaultId.hasNextPage}
...
export default KnowledgeinDevice